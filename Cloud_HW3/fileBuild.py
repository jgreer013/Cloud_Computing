...
def addEvent(name, start, end, days, eventString, start_date, end_date):
  res = ""
  res += eventString
  ...
  base_time = int(calendar.timegm(datetime.strptime(start_date,'%m/%d/%Y').timetuple()))
  fin_time = int(calendar.timegm(datetime.strptime(end_date,'%m/%d/%Y').timetuple()))
